...
try {
    ...
    if (str_contains($_POST['identifier'], 'Spieler')) {
        ...
        $stmt->bindParam(':FK_Team', $_POST['Teams']);
        $stmt->bindParam(':Trainingsbeginn', $_POST['Trainingsbeginn_date'] . ' ' . $_POST['Trainingsbeginn_time']);
        $stmt->bindParam(':Trainingsende', $_POST['Trainingsende_date'] . ' ' . $_POST['Trainingsende_time']);
        $wochentag = date('N', strtotime($_POST['Trainingsbeginn_date'])); // 'N' returns the ISO-8601 numeric representation of the day of the week (1 for Monday, 7 for Sunday)
