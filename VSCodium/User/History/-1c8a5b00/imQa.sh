...
toggle_wifi() {
    if is_wifi_on; then
        ...
        echo -e "Test2%{F#2193ff}󰀝%{F-}"
    else
        # Wi-Fi is currently off, turn it on
        iwctl device wlan0 set-property Powered on
        echo -e "Test3%{F#e06c75}󰀝%{F-}"
    fi
}
...
check() {
    if [ "$1" == "true" ]; then
        ...
        iwctl device wlan0 set-property Powered on
        echo -e "Test4%{F#e06c75}󰀝%{F-}"
    else
        # If not true, handle it accordingly (you can add more logic here if needed)
        returnValue= toggle_wifi
        echo $returnValue
    fi
}

# Call the function with an argument (e.g., "true" or "false")
check "$1"
