...
try {
  ...
  if(isset($_POST['deleteTeam'])) {
    ...
    $teamID = $parts_to_check[1];
    $trainingStart = $parts_to_check[2];
    
    echo "Trainer ID: $trainerId<br>";
    echo "Team Name: $teamID<br>";
    echo "Training Start: $trainingStart<br>";

    $stmt = $conn->prepare('DELETE FROM TrainerTeam WHERE FK_Trainer=:FK_TRAINER AND FK_Team=:FK_TEAM AND Trainingsbeginn=:TRAININGSBEGINN');
    $stmt->bindParam('FK_TRAINER', $trainerId);
    $stmt->bindParam('FK_TEAM', $teamID);
    $stmt->bindParam('TRAININGSBEGINN', $trainingStart);
    $stmt->execute();
  }
  else {
    echo "Failed";
